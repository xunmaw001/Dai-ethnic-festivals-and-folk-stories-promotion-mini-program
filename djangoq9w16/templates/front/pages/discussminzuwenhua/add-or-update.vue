	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussminzuwenhua`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussminzuwenhua`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussminzuwenhua`, this.ruleForm);
						}else{
							await this.$api.add(`discussminzuwenhua`, this.ruleForm);
						}
						uni.setStorageSync('discussminzuwenhuaCleanType',true);
