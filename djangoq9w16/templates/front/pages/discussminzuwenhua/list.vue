...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
				...
				<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"border":"0px solid #f5eae2","boxShadow":"0 0px 0px rgba(0,0,0,.3)","padding":"0px","margin":"0 0 40rpx","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}' v-for="(product,index) in list" :key="index">
				  ...
				  <view class="listmr" :style='{"width":"calc(100% - 0px)","padding":"0","margin":"8rpx 0 0","height":"auto"}'>
					...
					<view :style='{"width":"100%","padding":"16rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto","order":"5"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('discussminzuwenhua','修改')) || (!userid && isAuthFront('discussminzuwenhua','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('discussminzuwenhua','删除')) || (!userid && isAuthFront('discussminzuwenhua','删除'))" @click.stop="onDeleteTap(product.id)">
		...
		<button :style='{"border":"2rpx solid #fff","boxShadow":"0 4rpx 8rpx rgba(0,0,0,.1)","color":"#333","bottom":"120rpx","right":"128rpx","outline":"none","borderRadius":"100%","background":"#f8deea","width":"88rpx","lineHeight":"88rpx","fontSize":"28rpx","position":"fixed","height":"88rpx","zIndex":"9999"}' v-if="userid && isAuth('discussminzuwenhua','新增')" class="add-btn" @click="onAddTap()">新增</button>
		<button :style='{"border":"2rpx solid #fff","boxShadow":"0 4rpx 8rpx rgba(0,0,0,.1)","color":"#333","bottom":"120rpx","right":"128rpx","outline":"none","borderRadius":"100%","background":"#f8deea","width":"88rpx","lineHeight":"88rpx","fontSize":"28rpx","position":"fixed","height":"88rpx","zIndex":"9999"}' v-if="!userid && isAuthFront('discussminzuwenhua','新增')" class="add-btn" @click="onAddTap()">新增</button>
	</view>
</mescroll-uni>
</template>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`discussminzuwenhua`, params);
                } else {
                    res = await this.$api.list(`discussminzuwenhua`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('discussminzuwenhua', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`discussminzuwenhua`, searchForm);
                } else {
                    res = await this.$api.list(`discussminzuwenhua`, searchForm);
                }
				// 如果是第一页数据置空
