<template>
	<div class="addEdit-block" :style='{"padding":"30px","background":""}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="故事类型" prop="gushileixing">
					<el-input v-model="ruleForm.gushileixing" placeholder="故事类型" clearable  :readonly="ro.gushileixing"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="故事类型" prop="gushileixing">
					<el-input v-model="ruleForm.gushileixing" placeholder="故事类型" readonly></el-input>
				</el-form-item>
			</template>
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `gushileixing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.gushileixingCrossAddOrUpdateFlag = false;
						 this.parent.search();
