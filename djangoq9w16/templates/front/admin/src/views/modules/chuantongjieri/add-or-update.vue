<template>
	<div class="addEdit-block" :style='{"padding":"30px","background":""}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="节日名称" prop="jierimingcheng">
					<el-input v-model="ruleForm.jierimingcheng" placeholder="节日名称" clearable  :readonly="ro.jierimingcheng"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="节日名称" prop="jierimingcheng">
					<el-input v-model="ruleForm.jierimingcheng" placeholder="节日名称" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="节日类型" prop="jierileixing">
					<el-select :disabled="ro.jierileixing" v-model="ruleForm.jierileixing" placeholder="请选择节日类型" >
						<el-option
							v-for="(item,index) in jierileixingOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="节日类型" prop="jierileixing">
					<el-input v-model="ruleForm.jierileixing"
						placeholder="节日类型" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="由来年份" prop="youlainianfen">
					<el-input v-model="ruleForm.youlainianfen" placeholder="由来年份" clearable  :readonly="ro.youlainianfen"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="由来年份" prop="youlainianfen">
					<el-input v-model="ruleForm.youlainianfen" placeholder="由来年份" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'"  label="典故" prop="diangu">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.diangu" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.diangu" label="典故" prop="diangu">
                    <span :style='{"border":"1px solid #5497f2","padding":"0 40px","color":"#bbbbbb","borderRadius":"20px 40px","display":"inline-block","fontSize":"14px","lineHeight":"40px","fontWeight":"500"}' v-html="ruleForm.diangu"></span>
                </el-form-item>
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.diangu = this.ruleForm.diangu.replace(reg,'../../../djangoq9w16/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `chuantongjieri/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.chuantongjieriCrossAddOrUpdateFlag = false;
						 this.parent.search();
