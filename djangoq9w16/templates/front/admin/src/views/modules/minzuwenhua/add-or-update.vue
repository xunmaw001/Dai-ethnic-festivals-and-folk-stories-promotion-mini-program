	...
	<div class="addEdit-block" :style='{"padding":"30px","background":""}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="民族" prop="minzu">
					<el-input v-model="ruleForm.minzu" placeholder="民族" clearable  :readonly="ro.minzu"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="民族" prop="minzu">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'"  label="民族文化" prop="minzuwenhua">
					<editor 
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.minzuwenhua" label="民族文化" prop="minzuwenhua">
                    <span :style='{"border":"1px solid #5497f2","padding":"0 40px","color":"#bbbbbb","borderRadius":"20px 40px","display":"inline-block","fontSize":"14px","lineHeight":"40px","fontWeight":"500"}' v-html="ruleForm.minzuwenhua"></span>
                </el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'"  label="民族介绍" prop="minzujieshao">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.minzujieshao" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.minzujieshao" label="民族介绍" prop="minzujieshao">
                    <span :style='{"border":"1px solid #5497f2","padding":"0 40px","color":"#bbbbbb","borderRadius":"20px 40px","display":"inline-block","fontSize":"14px","lineHeight":"40px","fontWeight":"500"}' v-html="ruleForm.minzujieshao"></span>
                </el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='minzujieshao'){
							this.ruleForm.minzujieshao = obj[o];
							this.ro.minzujieshao = true;
							continue;
						}
						if(o=='fabushijian'){
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.minzuwenhua = this.ruleForm.minzuwenhua.replace(reg,'../../../djangoq9w16/upload');
        this.ruleForm.minzujieshao = this.ruleForm.minzujieshao.replace(reg,'../../../djangoq9w16/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `minzuwenhua/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.minzuwenhuaCrossAddOrUpdateFlag = false;
						 this.parent.search();
