		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.content" label="评论内容" prop="content">
					<span :style='{"border":"1px solid #5497f2","padding":"0 40px","color":"#bbbbbb","borderRadius":"20px 40px","display":"inline-block","fontSize":"14px","lineHeight":"40px","fontWeight":"500"}'>{{ruleForm.content}}</span>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.reply" label="回复内容" prop="reply">
					<span :style='{"border":"1px solid #5497f2","padding":"0 40px","color":"#bbbbbb","borderRadius":"20px 40px","display":"inline-block","fontSize":"14px","lineHeight":"40px","fontWeight":"500"}'>{{ruleForm.reply}}</span>
				</el-form-item>
...
export default {
    ...
    info(id) {
      this.$http({
        url: `discusschuantongjieri/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `discusschuantongjieri/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.discusschuantongjieriCrossAddOrUpdateFlag = false;
						 this.parent.search();
