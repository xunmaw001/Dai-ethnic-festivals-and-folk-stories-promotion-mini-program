...
export default {
  ...
  mounted(){
    ...
    this.chuantongjieriChat3();
    this.getminjiangushiCount();
    this.minjiangushiChat2();
  },
  created() {
    this.$nextTick(()=>{
		this.times()
...
//统计接口1
    chuantongjieriChat3() {
      this.$nextTick(()=>{
        ...
        var chuantongjieriChart3 = echarts.init(document.getElementById("chuantongjieriChart3"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                chuantongjieriChart3.setOption(option);
				

                  //根据窗口的大小变动图表
                window.onresize = function() {
                    chuantongjieriChart3.resize();
                };
            }
        });
      })
...
//统计接口1
    minjiangushiChat2() {
      this.$nextTick(()=>{
        ...
        var minjiangushiChart2 = echarts.init(document.getElementById("minjiangushiChart2"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                minjiangushiChart2.setOption(option);
				

                  //根据窗口的大小变动图表
                window.onresize = function() {
                    minjiangushiChart2.resize();
                };
            }
        });
      })
